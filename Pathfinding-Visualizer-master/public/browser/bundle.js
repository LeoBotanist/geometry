...
Board1.prototype.addEventListeners = function() {
  let board = this;
...
Board1.prototype.getNode = function(id) {
...
Board1.prototype.changeSpecialNode = function(currentNode) {
...
Board1.prototype.changeNormalNode = function(currentNode) {
...
Board1.prototype.drawShortestPath = function(targetNodeId, startNodeId, object) {
  let currentNode;
...
Board1.prototype.addShortestPath = function(targetNodeId, startNodeId, object) {
...
Board1.prototype.drawShortestPathTimeout = function(targetNodeId, startNodeId, type, object) {
  let board = this;
  let currentNode;
...
Board1.prototype.createMazeOne = function(type) {
...
Board1.prototype.clearPath = function(clickedButton) {
  if (clickedButton) {
...
Board1.prototype.clearWalls = function() {
  this.clearPath("clickedButton");
  Object.keys(this.nodes).forEach(id => {
    let currentNode = this.nodes[id];
    let currentHTMLNode = document.getElementById(id);
...
Board1.prototype.clearWeights = function() {
  Object.keys(this.nodes).forEach(id => {
    let currentNode = this.nodes[id];
    let currentHTMLNode = document.getElementById(id);
...
Board1.prototype.clearNodeStatuses = function() {
  Object.keys(this.nodes).forEach(id => {
    let currentNode = this.nodes[id];
    currentNode.previousNode = null;
    currentNode.distance = Infinity;
...
Board1.prototype.instantAlgorithm = function() {
  let weightedAlgorithms = ["dijkstra", "CLA", "greedy"];
  let unweightedAlgorithms = ["dfs", "bfs"];
  let success;
  if (this.currentAlgorithm === "bidirectional") {
...
Board1.prototype.redoAlgorithm = function() {
  this.clearPath();
  this.instantAlgorithm();
};

Board1.prototype.reset = function(objectNotTransparent) {
...
Board1.prototype.resetHTMLNodes = function() {
...
Board1.prototype.changeStartNodeImages = function() {
...
let counter = 1;
Board1.prototype.toggleTutorialButtons = function() {
...
Board1.prototype.toggleButtons = function() {
  ...
  if (!this.buttonsOn) {
    ...
    document.getElementById("actualStartButton1").style.backgroundColor = "rgb(185, 15, 15)";
  }


}

function Board2(height, width) {
  this.height = height;
  this.width = width;
  this.start = null;
  this.startNodes = [];
...
Board2.prototype.addEventListeners = function() {
  let board = this;
  for (let r = 0; r < board.height; r++) {
    for (let c = 0; c < board.width; c++) {
...
Board2.prototype.changeSpecialNode = function(currentNode) {
  let element = document.getElementById(currentNode.id), previousElement;
  if (this.previouslySwitchedNode) previousElement = document.getElementById(this.previouslySwitchedNode.id);
  if (currentNode.status !== "target" && currentNode.status !== "start" && currentNode.status !== "object") {
    if (this.previouslySwitchedNode) {
...
Board2.prototype.changeNormalNode = function(currentNode) {
  let element = document.getElementById(currentNode.id);
  let relevantStatuses = ["start", "target", "object"];
  let unweightedAlgorithms = ["dfs", "bfs"]
  if (!this.keyDown) {
...
Board2.prototype.drawShortestPath = function(targetNodeId, startNodeId, object) {
  let currentNode;
  if (this.currentAlgorithm !== "bidirectional") {
    currentNode = this.nodes[this.nodes[targetNodeId].previousNode];
    if (object) {
...
Board2.prototype.addShortestPath = function(targetNodeId, startNodeId, object) {
  let currentNode = this.nodes[this.nodes[targetNodeId].previousNode];
  if (object) {
    while (currentNode.id !== startNodeId) {
      this.objectShortestPathNodesToAnimate.unshift(currentNode);
...
Board2.prototype.drawShortestPathTimeout = function(targetNodeId, startNodeId, type, object) {
  let board = this;
  let currentNode;
  let secondCurrentNode;
  let currentNodesToAnimate;
...
Board2.prototype.createMazeOne = function(type) {
  Object.keys(this.nodes).forEach(node => {
    let random = Math.random();
    let currentHTMLNode = document.getElementById(node);
    let relevantClassNames = ["start", "target", "object"]
...
Board2.prototype.clearPath = function(clickedButton) {
  if (clickedButton) {
    let start = this.nodes[this.start];
    let target = this.nodes[this.target];
    let object = this.numberOfObjects ? this.nodes[this.object] : null;
...
Board2.prototype.clearWalls = function() {
  this.clearPath("clickedButton");
  Object.keys(this.nodes).forEach(id => {
    let currentNode = this.nodes[id];
    let currentHTMLNode = document.getElementById(id);
...
Board2.prototype.clearWeights = function() {
  Object.keys(this.nodes).forEach(id => {
    let currentNode = this.nodes[id];
    let currentHTMLNode = document.getElementById(id);
    if (currentNode.weight === 15) {
...
Board2.prototype.clearNodeStatuses = function() {
  Object.keys(this.nodes).forEach(id => {
    let currentNode = this.nodes[id];
    currentNode.previousNode = null;
    currentNode.distance = Infinity;
...
Board2.prototype.instantAlgorithm = function() {
  let weightedAlgorithms = ["dijkstra", "CLA", "greedy"];
  let unweightedAlgorithms = ["dfs", "bfs"];
  let success;
  if (this.currentAlgorithm === "bidirectional") {
...
Board2.prototype.redoAlgorithm = function() {
  this.clearPath();
  this.instantAlgorithm();
};

Board2.prototype.reset = function(objectNotTransparent) {
  this.nodes[this.start].status = "start";
  document.getElementById(this.start).className = "startTransparent";
  this.nodes[this.target].status = "target";
  if (this.object) {
...
Board2.prototype.resetHTMLNodes = function() {
  let start = document.getElementById(this.start);
  let target = document.getElementById(this.target);
  start.className = "start";
  target.className = "target";
...
Board2.prototype.changeStartNodeImages = function() {
  let unweighted = ["bfs", "dfs"];
  let strikethrough = ["bfs", "dfs"];
  let guaranteed = ["dijkstra", "astar"];
  let name = "";
...
let counter2 = 1;
Board2.prototype.toggleTutorialButtons = function() {
...
Board2.prototype.toggleButtons = function() {
  document.getElementById("refreshButton").onclick = () => {
    window.location.reload(true);
  }
  ...
  if (!this.buttonsOn) {
    ...
    document.getElementById("startButtonFloyd2").onclick = () => {
      document.getElementById("startButtonStart2").innerHTML = '<button id="actualStartButton2" class="btn btn-default navbar-btn" type="button">Visualize Floyd-Warshall!</button>'
      this.currentAlgorithm = "floyd_warshall";
      // this.currentHeuristic = "manhattanDistance"
      this.changeStartNodeImages();
    }
    ...
    document.getElementById("startButtonAddStart2").onclick = () => {

      let initial_start = this.start.slice(7).split("-");
      let r = parseInt(initial_start[0]), c = parseInt(initial_start[1])
      while(this.nodes[`board2_${r}-${c}`].status === "start") {
        r += 1;
      }
      this.nodes[`board2_${r}-${c}`].status = "start";
      document.getElementById(`board2_${r}-${c}`).className = "start";

      this.startNodes.push(`board2_${r}-${c}`);
      // console.log(this.startNodes);
      // console.log(this.start);
      let initial_r = parseInt(this.start.slice(7).split("-")[0]), initial_c = parseInt(this.start.slice(7).split("-")[1])
      this.startNodesRelative.push(`${r - initial_r}^${c - initial_c}`)
    ...
    document.getElementById("actualStartButton2").style.backgroundColor = "";
    ...
    document.getElementById("actualStartButton2").style.backgroundColor = "rgb(185, 15, 15)";
  }
